import{Timeline} from "@awayjs/scene"
...
import {ITimeline, MovieClip} from "@awayjs/scene";
import {EventDispatcher} from "@awayjs/core";
import {SwfTagCode} from "./SwfTags";
...
export class TimelineSWF implements ITimeline{

	static frameNavigationModel: FrameNavigationModel;

	static axClass: typeof TimelineSWF;//MovieClip;

	private static _callQueue: TimelineSWF[];//MovieClip [];
 ...
 */
	...
	private _frames: any[];//Shumway.SWF.SWFFrame[];
	private _frameScripts: any;
	private _scenes: any[];//Scene[];
	...
	private _sounds: any;//MovieClipSoundsManager;

	private _buttonFrames: any;//Shumway.MapObject<number>;
...
*/
	...
	private _seekToFrame(frame: number) {
		...
		var loaderInfo = (<any>this._symbol).loaderInfo;//SpriteSymbol
...
*/
	...
	gotoAndPlay(frame: any, scene: string = null): void {
		...
		if (arguments.length === 0 || arguments.length > 2) {
			//this.sec.throwError('ArgumentError', Errors.WrongArgumentCountError,'flash.display::MovieClip/gotoAndPlay()', 1,arguments.length);
		}
	...
	addFrameScript(frameIndex: number, script: FrameScript /*, ...*/): void {
		...
		if (numArgs & 1) {
			//this.sec.throwError('ArgumentError', Errors.TooFewArgumentsError, numArgs, numArgs + 1);
		}
